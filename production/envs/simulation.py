...
class ProductionEnv(Environment):
    ...
    def update_log(self):
        ...
        self.statistics["daily_production_EXPORT"].append(self.statistics["num_orders"] - self.statistics["delayed_orders"])
...
#        self.statistics["NUM_ORDERS_log"].append(self.statistics["num_orders"])
    ...
    def normilize_productivity(self):
        ...
        qnt = self.statistics["num_orders"] - self.statistics["delayed_orders"]
    ...
    def execute(self, actions):
        ...
        if self.episodes_counter == self.parameters["episodes"]*self.parameters["timesteps"]:
            ...
            export_dataset.to_excel(f"output_teste2\{datetime.now().strftime('%Y-%m-%d_%H-%M-%S')}.xlsx", index= False)
